...
export default function Dashboard() {
  ...
  const copyObsLink = (type: 'alert' | 'leaderboard') => {
    ...
    toast.success(type === 'alert' ? t('notificationLinkCopied') : t('leaderboardLinkCopied'));
  };
    ...
    <div className="min-h-screen bg-background text-foreground py-24 px-4">
        <div className="max-w-md mx-auto space-y-6">
            ...
            <div className="mb-6 flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold">{t('welcome')}<span className="text-primary">{profile?.display_name || 'User'}</span> 👋</h1>
            ...
            <div className="bg-card border border-border rounded-3xl p-6 relative overflow-hidden group shadow-sm">
                 ...
                 <div className="relative z-10">
                     <p className="text-muted-foreground text-sm font-medium mb-1">{t('balanceIdrx')}</p>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground font-medium">{t('copyLink')}</span>
                </div>
            ...
            <div>
                <div className="flex items-center gap-2 mb-4 text-muted-foreground">
                    ...
                    <span className="text-sm font-bold uppercase tracking-wider">{t('obsIntegration')}</span>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="bg-card border border-border p-4 rounded-2xl flex items-center justify-between group hover:border-primary/30 transition-colors shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-bold text-sm text-foreground">{t('notificationOverlay')}</div>
                                <div className="text-xs text-muted-foreground">{t('notificationOverlayDesc')}</div>
                            </div>
                        </div>
                    ...
                    <div className="bg-card border border-border p-4 rounded-2xl flex items-center justify-between group hover:border-primary/30 transition-colors shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-bold text-sm text-foreground">{t('leaderboardOverlay')}</div>
                                <div className="text-xs text-muted-foreground">{t('leaderboardOverlayDesc')}</div>
                            </div>
                        </div>
